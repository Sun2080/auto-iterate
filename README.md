...
| 模式 | 口令 | §6 停止条件命中时 |
|------|------|------|
| **有人值守**（默认） | `/loop 下一轮` | 连续同错 / idle → **停下问人**（留选择空间） |
| **无人值守** | `/loop 下一轮 · 无人值守` | 能 revert + pivot 就自救，同故障 3 次无果才 **硬停**（写清续跑消息） |

两模式下时间上限 / 30 轮大回 / 目标达成 / 预算耗尽都停，差异在"等人 vs 硬停"。详见 `iterate.md §6.1`。
