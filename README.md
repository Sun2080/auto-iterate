...
就这 5 个字。不带时间参数 = 动态节拍。停止条件由 `iterate.md §6` 自动兜底。
...
```text
请创建一个 thread automation，每 5 分钟回到当前线程，按 .claude/standards/iterate.md 跑下一轮；严格遵守 §6 停止条件
```
...
按当前官方文档，Codex CLI 的 slash commands 列表里没有和 Claude `/loop` 同名的命令；Codex 侧更适合用 Automations 承接循环。
...
| **手动跑一轮** | `下一轮` | `下一轮` |
| **会话内自动循环** | `/loop 下一轮` | `请创建一个 thread automation，按 .claude/standards/iterate.md 跑下一轮` |
...
**② 会话内自动循环**（稳定后再开）：
- **Claude Code**：优先 `/loop`。动态 `/loop 下一轮` 是“跑完立即起”的最佳形态；固定节拍用 `/loop 5m 下一轮`
- **Codex**：优先 thread automation。让它回到当前线程继续跑下一轮，保留当前上下文
