---
name: autopilot-mode
description: 无人参与的自主执行模式。用户只给 Mission + 上限,agent 自研究 / 自规划 / 自实现 / 自验证 / 自停。与 standards.md 叠加使用。
audience: host-project agent
when-loaded: 用户消息以 `自主:` / `自主跑:` / `自主模式:` / `托管:` 开头时加载本文件
---
...
- `自主:<mission>,限 <上限>`
- `自主跑:<mission>,限 <上限>` / `自主模式:...` / `托管:...`
- 等价表达:"放手做"、"无人参与"、"不参与"、"autopilot"
...
| 参数 | 格式 | 示例 |
|---|---|---|
| **Mission** | 一句话 | 做黄金交易辅助系统 |
| **上限** | 时间 OR commit 数 OR 预算(任一) | `4h` / `50 commit` / `$20 budget` |

**为什么强制上限**: Mission 判据由 agent 自填,没有上限 = 可能无限加功能烧钱。上限是防跑飞硬线,不是约束。

### 1.3 启动后立刻做的事

1. 回显一行:`[autopilot] Mission = <M>, 上限 = <U>, 现在进入 Bootstrap`
2. 开始 Round 1(§2)
