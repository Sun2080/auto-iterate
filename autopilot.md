...
用 `WebSearch` / `WebFetch` 做外部参照;涉及库 / API 时优先查官方文档或已接入的文档型 MCP(如 Context7):
...
**运行形态**: 紧凑连续循环(back-to-back),一轮跑完立即下一轮。**不** sleep / **不**自建计划任务 / **不**定时轮询 —— 靠 prompt cache 跨轮复用省 token,30min 冷启动那种坑走不到这里。
...
- UI 改动用浏览器能力真跑(Claude Code Chrome / 浏览器 MCP / 团队已有等价工具都行),没装就先装或改方案
- **重活派 subagent**:全局搜代码 / 读长文件 / 批量扫描 / 外部研究 → 委派给当前环境里最合适的研究型 subagent,主 context 只收摘要。主 agent 是指挥官不是苦力,长跑省 token 最大一招
...
- progress.md 记:`skip #N: <task>, fail 2x <err>, 换到 <new task>`(编号跟 §4.1 pivot 对齐,便于 HANDOFF 追溯)
...
- Success Criteria 候选**全部**进入"试过不成"状态 → **硬停**(§5)。"试过不成"含:
  - §4.1 pivot 过且无果
  - §4.0 skip 标 `[blocked]`
  - 已勾完(达成)
- 换句话说:剩余**未勾**且**未 blocked**候选为 0 就穷举
...
- <3>
**未做项**:
- <criteria 里没勾的,每条一行;§4.0 跳过的标 `[blocked: <原因一句话>]`,让用户一眼看出是"跳了"还是"没时间">
**续跑指令**: <一句话,用户回来复制即可恢复>
```
