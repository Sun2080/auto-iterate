...
## Round 31 @ 2026-04-21 · Claude / Codex 双栈适配 + 口径统一

**Trigger**: 用户同意修前轮 review 抓到的三处冲突，并追加新诉求：「这个能不能加工一下，可以同时 Claude 和 Codex 两家共用」。

这不是小修句子，而是一次**架构层收束**：必须把 `standards/` 里的平台私货抽出去，不然 README 改成双栈后，正文标准仍会把宿主拉回单平台。

**Modify**:
- `GOALS.md`
  - Status 从「达成所有 Success Criteria」改为**前 4 条已实证，第 5 条待真实宿主触发**，消除和 checkbox / R30 战报的硬冲突
  - Success Criteria 第 2 条改成 `CLAUDE.md` / `AGENTS.md` 双入口
  - Constraints 从「文件数 ≤ 10」改成「常驻文件 ≤ 12 + archive 不计常驻」，给新增 `templates/AGENTS.md.append` 和后续归档留空间
- `standards/code.md`
  - Part B 从「Opus 4.7 交互契约」抽象为**跨平台 Agent 交互契约**
  - 保留 Karpathy 4 原则不动；把文档 / API / 推理强度 / 验证 / token hygiene 改为 Claude/Codex 共性表述
- `standards/iterate.md`
  - §1.2 工具层改成平台自带 search/docs 工具的泛化说法
  - §2.3 Verify 从 Playwright MCP 独占改成 browser / computer-use / Playwright 类能力真跑
  - §5 节律重写为**手动 / 会话内自动 / 后台自动化**三角色；Claude `/loop` / `/schedule` 与 Codex thread / standalone automation 下沉为“平台映射示例”
  - §5.2 大回归档改为「主 `progress.md` 可保留最近 30 轮，或在仍可读时保留全量」，修复与 R30 实际做法冲突
  - §6 / §11 去掉 `CLAUDE.md` 单平台假设，改成「宿主指令文件」
- `README.md`
  - 首段和速查表改成 **Claude Code / Codex 双入口**
  - Step 2 改成「挂接到宿主指令文件」：Claude 用 `CLAUDE.md`，Codex 用 `AGENTS.md`
  - Step 5 改成平台适配层：手动通用；Claude 用 `/loop` / `/schedule`；Codex 用 Automations
  - Sources 补 OpenAI Codex docs
- `templates/CLAUDE.md.append`
  - 文案对齐新标准，并加一条：没有用户明确授权，不要自己开自动化
- `templates/AGENTS.md.append`
  - **新增** Codex 宿主 adapter，内容和 Claude 版同层级，只换载体
- `AGENT_MEMORY.md`
  - 更新 Decision「不做 `/iterate` skill」，扩到双平台
  - 新增 Decision「核心标准保持平台中性，平台差异放 adapter 层」

**Verify**:
- [x] 三处原冲突同时消失：`GOALS.md` 状态 vs checkbox、`iterate.md` 大回归档 vs `progress.md`、文件数 ≤ 10 vs 新模板/归档现实
- [x] Claude / Codex 都有明确入口：README 速查、Step 2、Step 5、templates 四处都能落地
- [x] `standards/` 仍然只讲「怎么工作」，没有把 `CLAUDE.md` / `AGENTS.md` / `/loop` / Automations 混成硬规则
- [x] 新增模板文件仅 1 个，常驻文件约束已同步放宽，避免再次自撞

**Status**: Keep.

**意义**:
- 本轮把 auto-iterate 从「Claude Code 专用知识包」抬到「Claude / Codex 共用的方法包」，但**没有**把核心标准稀释成空话
- 平台差异被压缩到最外层 adapter：宿主指令文件 + 自动化入口；中间的 code / iterate 协议继续共用
- 这条线以后也更稳：即使 Claude / Codex 的具体命令继续演进，改 README / templates 就够，不必再动 standards 本体

refs: memory#核心标准保持平台中性，平台差异放 adapter 层 · memory#不做 `/iterate` skill

---

## Round 32 @ 2026-04-21 · 独立使用说明书 · 12 小时无人值守场景补齐

**Trigger**: 用户继续追问实际操作层细节，核心痛点很具体：
- Codex 的 `thread automation` 中文到底怎么写
- 想“持续迭代，中间不要间隔 10 分钟”该怎么表达
- 想“无人值守自动迭代 12 小时”该怎么写
- 并明确要求：**把各种情况整理成一份单独使用说明书**

这说明 README 首屏虽然给了速查，但还不够承载“按场景决策 + 直接复制模板”这一层。需要一份独立的**操作手册**，而不是继续往 README 里塞例子。

**外部参照**（本轮直接用官方文档补齐平台差异）:
- Claude Code scheduled tasks 文档确认：
  - `/loop` 是 session-scoped；当前会话关闭就停
  - `/loop` 最小粒度 1 分钟；动态 `/loop` 由 Claude 在 1 分钟到 1 小时之间自选
  - 真正“电脑关了还继续跑”要用 cloud `routine`
- Claude routines 文档确认：
  - routine 跑在 Anthropic-managed cloud infrastructure
  - 电脑关了还能继续
  - CLI 入口是 `/schedule`
- Codex automations 文档确认：
  - `thread automation` = 绑定当前线程的 heartbeat-style recurring wake-up call，保留线程上下文
  - `standalone automation` = fresh run on a schedule
  - project-scoped automation 要求 app 在运行、项目在磁盘上可用
- Codex CLI slash commands 文档再次确认：
  - 有 `/compact`
  - **没有 `/loop`**
...
- **新增** `USAGE.md`
  - 单独写成“场景手册”，覆盖：
    ...
    - “电脑关了还继续跑”两平台差异
    - 直接可复制模板
  - 所有“12 小时”模板都要求写**绝对结束时间**，避免相对时间歧义
- `README.md`
  - 顶部设计原则下新增一行链接，把用户导到 `USAGE.md`

**Verify**:
- [x] `USAGE.md` 覆盖了用户当前连续 4 个问题的全部操作层答案，不再需要在 README 里到处翻
- [x] Claude / Codex 的差异点被明确拆开：`/loop`、`/schedule`、`thread automation`、`standalone automation`
- [x] “12 小时无人值守”包含了最关键的现实约束：Claude routine 可关电脑；Codex 当前公开文档未给出完全对等的云端本地项目方案
- [x] README 首屏已挂入口，后续回答用户时可以直接指向 `USAGE.md`

**Status**: Keep.

**意义**:
- 这轮不是再改标准，而是把“标准 → 实际操作”之间的最后一层落地补齐
- 对用户最有价值的不是多一个概念，而是**一份直接复制就能用的场景手册**
- 也顺手验证了双栈设计的好处：`standards/` 不动，新增一个 usage adapter 文档就能承接平台差异

refs: memory#铺信息 ≠ 用户能找到 · 高频口令要零前提 TL;DR

---

## Round 33 @ 2026-04-21 · 连续 vs 定时 口径硬分离

**Trigger**: 用户直接指出 `USAGE.md` 里我给的 5 分钟 / 15 分钟示例偏离核心诉求：

> “为什么是 15 分钟，和 5 分；我需要的是 连续不断，没有间隔”

这是个重要纠偏。上一轮虽然补了使用说明书，但把“无人值守 12 小时”的保守节拍示例放得太靠前，容易让人误以为**定时自动化 = 连续迭代**。用户这次等于在帮文档做概念去混淆。

**Modify**:
- `USAGE.md`
  - 顶部新增一节「**连续 vs 定时**」，先把概念拆开再讲工具
  - 明说：
    - Claude `/loop 下一轮` 最接近连续，但官方仍是动态调度，不是零间隔书面保证
    - Codex 当前公开文档未见完全对等的官方零间隔方案；automation 都是定时唤醒
  - Claude 12 小时、本机开着的默认模板从 `/loop 5m ...` 改成 **`/loop ...`**
  - Codex 新增一条“**当前会话里持续干，不走 automation**”模板，和“standalone automation = 无人值守定时跑”拆开
  - FAQ 重写成更硬口径：哪些是连续，哪些只是分钟级调度
- `README.md`
  - 顶部 `USAGE.md` 入口说明改成「连续跑 / 定时自动化」双词并列
  - Claude `/loop 下一轮` 补一句：最接近连续，但不是书面保证零间隔
  - Codex Automations 补一句：适合**定时**循环，不要再暗示它等于连续跑
  - 速查表把「会话内自动循环」改成「**最接近连续跑**」，避免把 Codex thread automation 和 Claude `/loop` 混成同义

**Verify**:
- [x] `USAGE.md` 顶部就能看到“连续 vs 定时”的硬区分，不用读到第 200 行才发现
- [x] Claude 默认推荐从 `/loop 下一轮` 出发，而不是先给 `5m`
- [x] Codex 文案不再把 thread / standalone automation 描述成“连续不断”
- [x] README 和 `USAGE.md` 口径重新对齐：Claude 有“最接近连续跑”的官方入口；Codex 公开文档暂未见完全等价物

**Status**: Keep.

**意义**:
- 这轮不是加内容，而是**去误导**
- 对“自动迭代”类工具来说，概念混淆比缺说明更危险：用户会按错模式，结果和预期完全不同
- 文档以后回答“12 小时”“无人值守”“连续不断”时，终于有了同一套硬口径

refs: memory#铺信息 ≠ 用户能找到 · 高频口令要零前提 TL;DR

---

## Round 34 @ 2026-04-21 · 模板改成完整可复制句子

**Trigger**: 用户在实际复制 `USAGE.md` 模板时，直接把一段带尖括号占位符的 `thread automation` 提示词贴出来，等于暴露了当前文档还有最后一层摩擦：

> 模板虽然信息够了，但还不是“拿起就贴”的成品。

上一轮已经把“连续 vs 定时”拆开了，这一轮要补的是**复制体验**：少一层占位符，少一次脑内替换，直接给完整句子。

**Modify**:
- `USAGE.md`
  - 顶部新增统一规则：所有完整 prompt 示例默认使用绝对时间 `2026-04-22 09:00 Asia/Shanghai`
  - 把多处 `"<结束时间，例如 ...>"` / `"<绝对结束时间>"` 占位写法，改成**完整可复制句子**
  - 保留“只改这一处结束时间”的提醒，但把提醒放到 prompt 外面，不再把占位符塞进 prompt 本体
- `README.md`
  - `USAGE.md` 入口补一句：模板默认就是完整绝对时间示例
  - Codex 速查里的 thread automation 例子从 `5 分钟` 收紧到 `1 分钟`，更贴近“高频继续”的实际预期

**Verify**:
- [x] `USAGE.md` 里关键模板不再出现尖括号时间占位符，复制即可用
- [x] 所有“12 小时 / 截止时间”类模板统一成绝对时间格式，避免“今晚”“明早”这类相对时间歧义
- [x] README 首屏入口和 `USAGE.md` 的模板口径重新对齐：都是“完整句子 + 只改结束时间”

**Status**: Keep.

**意义**:
- 这轮不是补概念，而是把文档从“说明书”再推近一步，变成真正的“复制即用手册”
- 对自动化 prompt 来说，尖括号占位符本身就是误操作源；把变量缩到只剩一个绝对时间，执行成本会明显更低
