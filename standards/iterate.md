...
**停下问人**（§6）: 连续 2 轮同错 / 连续 3 轮无 commit / 大回 30 轮 / 5 小时 / 目标达成 / 预算耗尽 ·· 外加 §7 跑偏（动作在 `GOALS.md` 找不到归属）·· **无人值守模式（§6.1）行为分叉**：能自救的 revert + pivot，同故障 3 次无果才硬停，不傻等人
...
**为什么分四条而不是合一**：粒度不同 → 查询成本不同。找「昨天的代码改动」查 git；找「上轮做了啥」查 progress；找「改这个模块要注意什么」查 AGENT_MEMORY。合一就只剩一个越来越长的文件，没法定位。
...
合计上限 **35 条 × 3 行 = 105 行正文**，加分节标题 / 顶部说明 / 限额注释约 ~130 行。总长 ≤ 200 行硬上限（留白给临时膨胀）。

### 4.4 剪枝（强制）

- **每 6 轮小结时**：执行「prune AGENT_MEMORY.md」步骤，合并重复、删除已不适用的
- **每 30 轮大回时**：更激进地压缩；长期不被引用（下文说明）的转到 `archive/agent-memory-YYYY-MM.md`，不删但移出主文件
...
| 粒度 | 周期 | 动作 |
|------|------|------|
| 轮 | 1-20 min（按任务规模，见下） | Modify → Commit → Verify → Keep/Revert |
| 小回 | 6 轮 | 回顾 · 整理 · 清理 · 规划（见 §5.1） |
| 大回 | 30 轮 | 停下，产出战报，等人类指示（见 §5.2） |

**数字参数化，不写死**。量化项目可能 5min/轮，文档项目可能 20min/轮；**短任务（每轮 <2min）用 `/loop 1m`** —— 借 prompt cache 5min TTL 连续命中比长周期更省（10min 周期每轮都冷启）。官方 cron 粒度 1 分钟，`30s` 会被向上舍入成 `1m`，不是独立档位。默认值来自用户；skill 调用时可覆盖。

**`/loop Xm` 不会堆积**：Claude Code 官方是**串行等待 + 跳过补偿**（[scheduled-tasks](https://code.claude.com/docs/en/scheduled-tasks.md)）—— 上一轮没跑完，下一轮 prompt 入队等，不并发；等期间又过 tick 则跳过不补打。想「跑完立即起下一轮、完全无并发」，用**动态 `/loop <prompt>`**（不带时间参数）：Claude Code 让 agent 自调速 1min-1h（内部调 `ScheduleWakeup`）。动态模式是 Claude Code 原生机制，和 §6 末段「agent 不自起 cron」不冲突 —— **是用户起的 `/loop`，agent 在其框架内自调速**。
...
停止行为**按值守模式分叉**（见 §6.1 声明方式）。默认有人值守。

| 维度 | 阈值 | 有人值守 | 无人值守 |
|------|------|------|------|
| 连续同错 | 同一验证失败 ≥ 2 轮 | 停下问人，附错误摘要 | revert + pivot 到 GOALS 已对齐的候选动作；**同一故障点 pivot ≥ 3 次仍错 → 硬停** |
| Idle detection | 连续 3 轮无 commit | 标记「卡住」，停下问人 | revert 到上个稳定态 + 换候选 micro-goal；**候选穷举无果 → 硬停** |
| 大回终点 | 30 轮 | 按 §5.2 停，等人类 | 按 §5.2 硬停（写"待审大回初稿"到 progress.md） |
| 时间上限 | 默认 5 小时 | 产中间战报，等人类 | **硬停**（产终报，不等） |
| 目标达成 | GOALS.md 全部判据满足 | 产终报，停 | 同 |
| 预算耗尽 | Task budget 告警 | 优雅收尾，不开新 commit | 同 |
...
**硬停时的标准消息**（无人值守）：
```
HARD STOP @ round N · reason: <维度>
- 发生了什么: <一句话>
- 停在哪: <文件:行 / commit sha / 错误摘要>
- 自救尝试: <已 pivot N 次，各次动作 + 结果>
- 怎么续: <用户回来后一句话可恢复的指令 / 前提条件>
```

不写长篇大论 —— 有人值守留选择空间，无人值守留恢复路径。

**正面停止**（目标达成 / 预算耗尽）共用 HARD STOP 模板：`自救尝试` 填 `N/A（非故障停止）`；`怎么续` 填「等新信号 / 扩 GOALS / tag release」等正向恢复指令。

### 6.1 值守模式

起循环时显式声明，agent 从 prompt 识别：

- **默认 有人值守**：`/loop 下一轮`
- **无人值守**：`/loop 下一轮 · 无人值守`（或等价表达："过夜跑"、"不等我反馈"、"autopilot"）

**无人值守三原则**：
1. **自救优先**：能 revert + pivot 的条件不停下空等（§6 表格"无人值守"列）
2. **自救有限**：同一故障点 pivot ≥ 3 次仍无果 → 硬停，不无限试
3. **硬停即清晰交接**：progress.md 尾写清 HARD STOP 消息，不 ScheduleWakeup；用户回来一眼知道从哪续

**硬线**：无人值守 ≠ 允许跑偏。§7 跑偏检测仍适用 —— pivot 目标必须在 GOALS.md 已有条目里，**绝不自创新方向**。找不到 GOALS 归属 → 硬停，不发挥创造力。

**大回停顿后的无人值守续跑**（可选）：30 轮大回停下后，若用户想延续循环（比如过夜跑），**用户**可以用 `/schedule` 注册 cron trigger 定时重启下一轮。**agent 不自己起** `CronCreate` —— 节奏由人决定，这是 GOALS non-goals 的硬线。（无人值守模式下 agent 在 `/loop` 框架内 `ScheduleWakeup` 自调速仍允许，见 §5。）
