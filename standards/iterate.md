...
**节律**: 手动或平台自动化触发 · 6 轮小回剪枝 · 30 轮大回归档
...
**工具**
- 平台自带的 web/search/docs 工具（如 Claude 的 WebSearch/WebFetch、Codex 的 web/docs/search）
...
- 测试通过 / 编译通过 / lint 通过
- 目标指标改善（例如延迟、准确率、错误数）
- **UI / 前端改动**：用平台已有的 browser / computer-use / Playwright 类能力真跑。最小三连：
  - 打开改动页
  - 看 DOM / 截图 / 可视结果
  - 看 console / runtime error
  编译过 + 类型过 ≠ UI 可用。必须跑一遍真实交互。
...
3. **不是 code.md 已覆盖的** —— Karpathy 4 原则 / Agent 交互契约不重复
...
| 触发层 | 周期 | 动作 |
|------|------|------|
| 手动轮 | 用户显式触发 | Modify → Commit → Verify → Keep/Revert |
| 会话内自动循环 | 1-10 min（按平台能力） | 在当前上下文里继续跑下一轮 |
| 平台级后台自动化 | 5 min-1 day（按平台能力） | 独立或半独立地按节拍执行 |
| 小回 | 6 轮 | 回顾 · 整理 · 清理 · 规划（见 §5.1） |
| 大回 | 30 轮 | 停下，产出战报，等人类指示（见 §5.2） |

**先手动，再自动化**。自动化前，至少手动跑通 1 轮，确认 prompt、验证方式、改动粒度都对。

**数字参数化，不写死**。短任务常用 1-5 min，长任务常用 10-20 min；追求的是不堆积、可停机、方便验证，不是零间隔本身。

**平台映射示例**：
- Claude Code：手动 `下一轮`；会话内自动循环常见是 `/loop`；后台定时常见是 `/schedule` / routines
- Codex：手动 `下一轮`；会话内自动循环常见是 thread automation；后台定时常见是 standalone automation

标准只规定“手动 / 同上下文自动 / 后台自动”这三个角色，不写死必须是哪家的命令。
...
3. **归档**：过去 30 轮 progress 可以压缩为摘要放 `progress-archive/round-<start>-<end>.md`；主 `progress.md` 保留最近 30 轮，或在仍可读时保留全量
...
**大回停顿后的无人值守续跑**（可选）：30 轮大回停下后，若用户想延续无人值守循环（比如过夜跑），**用户**可以显式要求使用平台自动化。Claude 常见是 `/schedule` / routines；Codex 常见是 standalone automation。**agent 不自己起**计划任务 —— 节奏由人决定，这是 GOALS non-goals 的硬线。
...
2. **在宿主指令文件里记一张映射表**（见下方示例），告诉 agent「我们家的 X 对应 auto-iterate 的 Y」。
...
Claude 宿主把这段放进 `CLAUDE.md`；Codex 宿主把同一段放进 `AGENTS.md`。双栈项目可以两边都放，内容保持同一份。
