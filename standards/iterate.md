...
| 粒度 | 周期 | 动作 |
|------|------|------|
| 轮 | 30s-20 min（按任务规模，见下） | Modify → Commit → Verify → Keep/Revert |
| 小回 | 6 轮 | 回顾 · 整理 · 清理 · 规划（见 §5.1） |
| 大回 | 30 轮 | 停下，产出战报，等人类指示（见 §5.2） |

**数字参数化，不写死**。量化项目可能 5min/轮，文档项目可能 20min/轮；**短任务（每轮 <2min）用 `/loop 1m` 甚至 30s** —— 借 prompt cache 5min TTL 连续命中比长周期更省（10min 周期每轮都冷启）。默认值来自用户；skill 调用时可覆盖。
